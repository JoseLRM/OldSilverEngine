...
namespace sv {
	...
	static GPUBuffer*	g_Buffer_ToneMapping = nullptr;
...
Output main(Input input) 
{
	...
	// HDR to LDR
	output.color.rgb = 1.f - exp(-output.color.rgb * exposure);
	...
	return output;
...

		{
			...
			desc.size = sizeof(ToneMappingData);
			desc.pData = nullptr;

			svCheck(graphics_buffer_create(&desc, &g_Buffer_ToneMapping));
			graphics_name_set(g_Buffer_ToneMapping, "CBuffer_ToneMapping");
		}
	...
	{
		svCheck(graphics_destroy(g_RenderPass_ToneMapping));
		svCheck(graphics_destroy(g_PS_ToneMapping));
		svCheck(graphics_destroy(g_Sampler_ToneMapping));
		svCheck(graphics_destroy(g_Buffer_ToneMapping));
	...
	void PostProcessing::toneMapping(GPUImage* img, GPUImageLayout imgLayout, GPUImageLayout newLayout, f32 exposure, CommandList cmd)
	{
		...
		graphics_constantbuffer_bind(g_Buffer_ToneMapping, 0u, ShaderType_Pixel, cmd);

		ToneMappingData data;
		data.exposure = exposure;

		graphics_buffer_update(g_Buffer_ToneMapping, &data, sizeof(f32), 0u, cmd);

...
namespace sv {
    ...
    struct IndexedListIterator {
		...
		IndexedListEntry<T>* begin = NULL;
		IndexedListEntry<T>* ptr = NULL;
		IndexedListEntry<T>* next = NULL;
		IndexedListEntry<T>* end = NULL;

		u32 get_index() {
			return u32(ptr - begin);
		}
		...
		IndexedListIterator& operator++() noexcept { _next(); return *this; }
		IndexedListIterator operator++(int) noexcept { IndexedListIterator temp = *this; _next(); return temp; }

		IndexedListIterator(IndexedListEntry<T>* begin, IndexedListEntry<T>* ptr, IndexedListEntry<T>* end) : begin(begin), next(ptr), end(end) {_next();}
    };

	template<typename T>
    struct ConstIndexedListIterator {

		const IndexedListEntry<T>* begin = NULL;
		const IndexedListEntry<T>* ptr = NULL;
		const IndexedListEntry<T>* next = NULL;
		const IndexedListEntry<T>* end = NULL;
		...
		ConstIndexedListIterator& operator++() noexcept { _next(); return *this; }
		ConstIndexedListIterator operator++(int) noexcept { ConstIndexedListIterator temp = *this; _next(); return temp; }

		ConstIndexedListIterator(const IndexedListEntry<T>* begin, const IndexedListEntry<T>* ptr, const IndexedListEntry<T>* end) : begin(begin), next(ptr), end(end) {_next();}
    };

    template<typename T>
    struct IndexedList {

		using Entry = IndexedListEntry<T>;
		...
		ConstIndexedListIterator<T> begin() const {
			...
			return ConstIndexedListIterator<T>(_data, it, end);
		}

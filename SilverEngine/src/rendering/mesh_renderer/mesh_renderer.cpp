...
namespace sv {
	...
	static Shader* g_Shader_LightingVertex = nullptr;
	static Shader* g_Shader_LightingPixel = nullptr;
...
SurfaceOutput main(UserSurfaceInput input)
{
	return meshSurface(input);
		...
		// Lighting shaders
		{
			svCheck(graphics_shader_compile_fastbin("MeshLightingVertex", ShaderType_Vertex, &g_Shader_LightingVertex, R"(
#include "core.hlsl"
struct Output {
...
Output main(u32 id : SV_VertexID)
{
	...
	return output;
			...
			svCheck(graphics_shader_compile_fastbin("MeshLightingPixel", ShaderType_Pixel, &g_Shader_LightingPixel, R"(
#include "core.hlsl"
struct Input {
	float2 texCoord : FragTexCoord;
};
...
SurfaceOutput meshSurface(UserSurfaceInput input)
{
	...
	void MeshRenderer::drawMeshes(GPUImage* renderTarget, GBuffer& gBuffer, CameraBuffer& cameraBuffer, FrameList<MeshInstanceGroup>& meshes, FrameList<LightInstance>& lights, bool optimizeLists, bool depthTest, CommandList cmd)
	{
		...
		// Wait to finish the shadow mapping
		task_wait(threadContext);
		...
		// LIGHTING PASS
		{
			...
			graphics_shader_bind(g_Shader_LightingVertex, cmd);
			graphics_shader_bind(g_Shader_LightingPixel, cmd);

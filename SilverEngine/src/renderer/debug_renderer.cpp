...
#define SV_PARSE_BATCH() sv::DebugRendererBatch& batch = sv::debug_batches[cmd]
    ...
    constexpr u32 DEBUG_QUAD_COUNT = 100u;
    constexpr u32 DEBUG_LINE_COUNT = 100u;
    constexpr u32 DEBUG_TRIANGLE_COUNT = 100u;
	...
	if (batch.quads.size()) {
	    ...
	    GPUBuffer* buffer = get_batch_buffer(DEBUG_QUAD_COUNT * sizeof(DebugVertex_Solid) * 6u, cmd);
		...
		*it = { v4_f32(v2), q.color }; ++it;
		...
		if (it == end) {
		    ...
		    graphics_buffer_update(buffer, data, DEBUG_QUAD_COUNT * sizeof(DebugVertex_Solid) * 6u, 0u, cmd);

		    graphics_renderpass_begin(renderpass, att, nullptr, 1.f, 0u, cmd);
		    graphics_draw(DEBUG_QUAD_COUNT * 6u, 1u, 0u, 0u, cmd);
	    ...
	    if (it != data) {
		graphics_buffer_update(buffer, data, u32((it - data) * sizeof(DebugVertex_Solid)), 0u, cmd);

		graphics_renderpass_begin(renderpass, att, nullptr, 1.f, 0u, cmd);
		graphics_draw(u32(it - data), 1u, 0u, 0u, cmd);
		graphics_renderpass_end(cmd);
	    }
	}
	...
	if (batch.lines.size()) {
	    ...
	    GPUBuffer* buffer = get_batch_buffer(DEBUG_LINE_COUNT * sizeof(DebugVertex_Solid) * 2u, cmd);
		...
		*it = { v4_f32(v1), l.color }; ++it;
		...
		if (it == end) {
		    ...
		    graphics_buffer_update(buffer, data, DEBUG_LINE_COUNT * sizeof(DebugVertex_Solid) * 2u, 0u, cmd);

		    graphics_renderpass_begin(renderpass, att, nullptr, 1.f, 0u, cmd);
		    graphics_draw(DEBUG_LINE_COUNT * 2u, 1u, 0u, 0u, cmd);
		    graphics_renderpass_end(cmd);
	...
	if (batch.triangles.size()) {
	    ...
	    GPUBuffer* buffer = get_batch_buffer(DEBUG_TRIANGLE_COUNT * sizeof(DebugVertex_Solid) * 3u, cmd);
		...
		*it = { v4_f32(v2), t.color }; ++it;
		...
		if (it == end) {
		    ...
		    graphics_buffer_update(buffer, data, DEBUG_TRIANGLE_COUNT * sizeof(DebugVertex_Solid) * 3u, 0u, cmd);

		    graphics_renderpass_begin(renderpass, att, nullptr, 1.f, 0u, cmd);
		    graphics_draw(DEBUG_TRIANGLE_COUNT * 3u, 1u, 0u, 0u, cmd);
		    graphics_renderpass_end(cmd);
	...
	if (batch.mesh_wireframes.size()) {
		...
		graphics_draw_indexed((u32)t.mesh->indices.size(), 1u, 0u, 0u, 0u, cmd);
		graphics_renderpass_end(cmd);
	    }
	}
    }

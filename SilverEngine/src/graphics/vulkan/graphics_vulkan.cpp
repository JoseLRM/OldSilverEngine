...
namespace sv {
	...
	void graphics_vulkan_device_prepare(GraphicsDevice& device)
	{
		...
		device.swapchain_acquire_image	= graphics_vulkan_swapchain_acquire_image;
		device.gpu_wait					= graphics_vulkan_gpu_wait;
		device.frame_begin				= graphics_vulkan_frame_begin;
		device.commandlist_submit		= graphics_vulkan_commandlist_submit;
	...
	GPUImage& graphics_vulkan_swapchain_acquire_image()
	{
		...
		g_API->swapChain.backBuffer.image = g_API->swapChain.images[g_API->imageIndex].image;
		g_API->swapChain.backBuffer.renderTargetView = g_API->swapChain.images[g_API->imageIndex].view;
		g_API->swapChain.backBuffer.ID = g_API->swapChain.images[g_API->imageIndex].ID;

		g_API->swapChain.backBuffer.type = GraphicsPrimitiveType_Image;
		g_API->swapChain.backBuffer.dimension = 2u;
		g_API->swapChain.backBuffer.format = graphics_vulkan_parse_format(g_API->swapChain.currentFormat);
		g_API->swapChain.backBuffer.width = g_API->swapChain.currentExtent.width;
		g_API->swapChain.backBuffer.height = g_API->swapChain.currentExtent.height;
		g_API->swapChain.backBuffer.depth = 1u;
	...
	void graphics_vulkan_frame_begin()
	{
		...
		if (now - g_API->lastTime >= SV_GFX_VK_UNUSED_OBJECTS_TIMECHECK) {

...
namespace sv {
    ...
    SV_AUX void add_glyph(char c, stbtt_fontinfo& info, f32 heightScale, List<TempGlyph>& glyphs)
    {
	...
	std::sort(glyphs.data(), glyphs.data() + glyphs.size(), 
	...
	while (it != end) {
	    it = process_text_line(it, u32(count - (it - text)), max_line_width / (font_size / aspect), *pFont);
	    ++lines;
	}

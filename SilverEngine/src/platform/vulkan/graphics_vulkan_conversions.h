...
namespace sv {
    ...
    inline void graphics_vulkan_parse_spirvstruct(spirv_cross::Compiler& comp, const spirv_cross::TypeID& ID, List<ShaderAttribute>& attributes)
    {
		...
		while (true) {
			...
			std::string name = std::move(comp.get_member_name(ID, i));
			if (name.empty()) break;
			...
			if (type.basetype == spirv_cross::SPIRType::BaseType::Struct) {
				...
				ShaderAttribute& a = attributes.back();
				...
				if (attributes.size() == 1u) a.offset = 0u;
					...
					if (a.offset / 16u != (a.offset + SV_MIN(typeSize - 1u, 15u)) / 16) a.offset += 16 - (a.offset % 16);
				}
			}

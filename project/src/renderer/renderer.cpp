...
#define CREATE_RENDERPASS(name, renderpass) svCheck(graphics_renderpass_create(&desc, &renderpass));
	...
	Result renderer_initialize()
	{
		...
		svCheck(font_create(font_opensans, SV_SYS("system/fonts/OpenSans/OpenSans-Regular.ttf"), 128.f, 0u));
		svCheck(font_create(font_console, SV_SYS("system/fonts/Cousine/Cousine-Regular.ttf"), 128.f, 0u));

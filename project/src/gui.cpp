...
namespace sv {
	...
	*/
	...
	SV_INLINE static void write_text(GUI_internal& gui, const char* text)
	{
		...
		write_buffer(gui, &text_size, sizeof(size_t));

		if (text_size) {

			write_buffer(gui, text, text_size);
		}
	}
	...
	void gui_end(GUI* gui_)
	{
		...
		while (it != end) {
			...
			case GuiWidgetType_Button:
			{
				...
				button.text_size = _read<size_t>(it);

				if (button.text_size) {
					button.text = (const char*)it;
					it += button.text_size;
				}
				else button.text = nullptr;
	...
	SV_INLINE static void* find_widget(GUI_internal& gui, GuiWidgetType type, u64 id)
	{
		GuiWidget res;
	...
	void gui_begin_container(GUI* gui_, u64 id, GuiCoord x0, GuiCoord x1, GuiCoord y0, GuiCoord y1, const GuiContainerStyle& style)
	{
		...
		GuiContainer* state = (GuiContainer*)find_widget(gui, GuiWidgetType_Container, id);
		begin_parent(gui, state, id);
	}
	...
	bool gui_button(GUI* gui_, const char* text, u64 id, GuiCoord x0, GuiCoord x1, GuiCoord y0, GuiCoord y1, const GuiButtonStyle& style)
	{
		...
		GuiButton* state = (GuiButton*) find_widget(gui, GuiWidgetType_Button, id);

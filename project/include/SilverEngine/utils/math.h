...
namespace sv {
    ...
    SV_INLINE u32 _height(u32 seed, u32 offset)
    {
	return seed * 24332 * (offset + 0x9e3779b9 + (u32(offset) << 6) + (u32(offset) >> 2));
    }

    SV_INLINE f32 _random(u32 seed)
    {
	return math_random_f32(seed);
    }
	...
	f32 p0 = f32(offset) + _random(_point(seed, offset));
	f32 h0 = _random(_height(seed, offset));
	
	f32 p1;
	if (n < p0) {
		
	    --offset;
	    p1 = f32(offset) + _random(_point(seed, offset));

	    f32 pos = (n - p1) / (p0 - p1);
	    pos = cos(PI - pos * PI) * 0.5f + 0.5f;
		
	    f32 h1 = _random(_height(seed, offset));

...
namespace sv {
	...
	void matsys_material_bind(Material* pInternal, SubShaderID subShaderID, CommandList cmd)
	{
		...
		for (u32 i = 0; i < info.textures.size(); ++i) {
			...
			u32 bindSlot = info.textureSlots[i].i[subShaderID];
		...
		// Bind Buffer
		u32 slot = info.bufferBindings[subShaderID];
		if (slot != u32_max) {
	...
	Result Material_internal::recreate(ShaderLibrary_internal& lib)
	{
		...
		u32 oldSubShaderCount = old.type->subShaderCount;

		// Attributes
		for (u32 i = 0u; i < oldInfo.attributes.size(); ++i) {

			const MaterialAttribute& oldAtt = oldInfo.attributes[i];

			// Find if it exist
			for (u32 j = 0u; j < newInfo.attributes.size(); ++j) {
				...
				if (oldAtt.type == newAtt.type && strcmp(oldAtt.name.c_str(), newAtt.name.c_str()) == 0) {
					...
					u32 typeSize = graphics_shader_attribute_size(att.type);
					XMMATRIX rawData;

					// Get the raw value
					const SubShaderIndices& srcIndices = oldInfo.attributeOffsets[i];

					for (u32 w = 0; w < oldSubShaderCount; ++w) {

						u32 index = srcIndices.i[w];
					...
					for (u32 w = 0; w < ShaderType_GraphicsCount; ++w) {

						u32 index = dstIndices.i[w];
		...
		// Textures
		for (u32 i = 0u; i < oldInfo.textures.size(); ++i) {

			const std::string& oldTexName = oldInfo.textures[i];

			for (u32 j = 0u; j < newInfo.textures.size(); ++j) {
	...
	Result matsys_material_attribute_set(Material* pInternal, ShaderAttributeType type, const char* name, const void* data)
	{
		...
		u32 subShaderCount = mat.shaderLibrary->type->subShaderCount;
		...
		for (size_t i = 0u; i < info.attributes.size(); ++i) {
			...
			if (att.type == type && strcmp(att.name.c_str(), name) == 0) {
				...
				for (u32 j = 0; j < subShaderCount; ++j) {

					u32 index = info.attributeOffsets[i].i[j];
					if (index != u32_max) {
	...
	Result matsys_material_texture_set(Material* pInternal, const char* name, GPUImage* image)
	{
		...
		for (u32 i = 0u; i < info.textures.size(); ++i) {

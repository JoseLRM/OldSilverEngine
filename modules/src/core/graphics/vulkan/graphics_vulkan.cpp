...
namespace sv {
	...
	void graphics_vulkan_renderpass_begin(CommandList cmd_)
	{
		...
		renderPass.beginInfo.renderArea.extent.width = state.attachments[0]->width;
		renderPass.beginInfo.renderArea.extent.height = state.attachments[0]->height;
	...
	Result graphics_vulkan_renderpass_create(RenderPass_vk& renderPass, const RenderPassDesc& desc)
	{
		...
		renderPass.beginInfo.clearValueCount = u32(desc.attachments.size());
